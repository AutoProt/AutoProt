...
namespace AutoProt
{
    ...
    {
        public ConnectionStateChangedEventArgs(ConnectionState connectionState, string path)
        {
    ...
    public class NetFileSystemWatcher : FileSystemWatcher
    {
        ...
        private void SetConnectionState(ConnectionState newState)
        {
            if (ConnectionState != newState)
            {
                ...
                if (stateChanged)
                    OnConnecionStateChanged(new ConnectionStateChangedEventArgs(newState,this.Path));
            }
            else
            {
                InStateCounter++;
                if (newState == ConnectionState.Connected)
                    OnConnecionStateChanged(new ConnectionStateChangedEventArgs(ConnectionState.Reconnected, this.Path));
            }
        }
